...
Configure `STRIPE_SECRET_KEY` and the Standard/Pro Price IDs, particularly when using existing Stripe Payment Links. New app-created Checkout subscriptions also carry `app=dataprovido` and plan metadata. Configure a stable random `COOKIE_SECRET` shared by workers. It is used to encrypt and authenticate OAuth session contents. The historical public example secrets are rejected in favor of a random process-local fallback, which changes on restart and is unsuitable for multiple workers. Changing the encryption secret invalidates old sessions and requires sign-in again.
